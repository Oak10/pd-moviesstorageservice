pipeline {
    ...
    environment {
        ...
        DOCKERHUB_REPO='moviesstorageservice'
    ...
    stages {
        ...
        stage('Deploy - DEV'){
            ...
            steps {
                sshagent(['ubuntulocalhost']) {
                    sh "ssh -l $DEPLOY_USER $DEPLOY_HOST 'ansible-playbook -i pd-ansible/inventory/dev/hosts.yml --diff pd-ansible/playbooks/moviesstorageservice-swarm.yml --extra-vars \"moviesstorageservice_tag=dev-$BUILD_NUMBER\"'"
                } 
            }
        }
    }
